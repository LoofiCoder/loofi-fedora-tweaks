...
def count_tab_files() -> tuple[int, list[str]]:
    ...
    tabs = sorted(
        p.stem for p in UI_DIR.glob("*_tab.py") if p.is_file() and p.stem != "base_tab"
    )
...
def count_utils_modules() -> int:
    ...
    return len(
        [p for p in UTILS_DIR.glob("*.py") if p.is_file() and p.stem != "__init__"]
    )
...
def read_roadmap_versions() -> dict[str, str]:
    ...
    # Find ACTIVE version
    active_match = re.search(r"\|\s*v([\d.]+)\s*\|[^|]*\|\s*ACTIVE\s*\|", text)
    if active_match:
        result["active_version"] = active_match.group(1)

    # Find NEXT version
    next_match = re.search(r"\|\s*v([\d.]+)\s*\|[^|]*\|\s*NEXT\s*\|", text)
    if next_match:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Project stats introspection engine")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--markdown", action="store_true", help="Also generate .project-stats.md"
    )
    parser.add_argument(
        "--json-only",
        action="store_true",
