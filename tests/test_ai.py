...
class TestOllamaManager(unittest.TestCase):
    ...
    @patch("utils.ai.cached_which", return_value="/usr/bin/ollama")
    ...
    def test_start_service_success(
        self, mock_is_installed, mock_is_running, mock_popen
    ):
        result = OllamaManager.start_service()
        self.assertTrue(result.success)
    ...
    @patch("utils.ai.OllamaManager.is_installed", return_value=True)
    def test_stop_service_fallback_pkill(
        self, mock_is_installed, mock_is_running, mock_run
    ):
        mock_run.side_effect = [MagicMock(returncode=1), MagicMock(returncode=0)]
        result = OllamaManager.stop_service()
        self.assertTrue(result.success)
...
class TestLlamaCppManager(unittest.TestCase):
    ...
    @patch("utils.ai.cached_which", return_value="/usr/bin/dnf")
    ...
    def test_install_detects_dnf_package(self, mock_is_installed, mock_run, mock_pm, mock_which):
        ...
        self.assertFalse(result.success)
        self.assertIn("pkexec dnf install llama-cpp", result.message)


class TestAIConfigManager(unittest.TestCase):
    """Coverage tests for AIConfigManager."""

    @patch("utils.ai.cached_which", return_value=None)
    ...
    @patch("utils.ai.cached_which", return_value="/usr/bin/nvidia-smi")
