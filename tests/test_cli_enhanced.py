...
class TestCLIInfoCommand(unittest.TestCase):
    ...
    def test_info_traditional_system(
        self, mock_power, mock_pm, mock_atomic, mock_pending
    ):
...
class TestCLITweakCommand(unittest.TestCase):
    ...
    @patch("subprocess.run")
    @patch(
        "cli.main.TweakOps.set_power_profile",
...
class TestCLIProcessesCommand(unittest.TestCase):
    ...
    @patch(
        "cli.main.ProcessManager.get_process_count",
...
class TestCLINetmonCommand(unittest.TestCase):
    ...
    @patch(
        "cli.main.NetworkMonitor.get_bandwidth_summary",
        return_value={
            "total_sent": 5000,
            "total_recv": 10000,
            "total_send_rate": 100.0,
            "total_recv_rate": 200.0,
        },
    )
...
class TestCLIAdvancedCommand(unittest.TestCase):
    ...
    @patch(
        ...
        return_value=OperationResult(success=True, message="BBR enabled"),
    )
    def test_advanced_bbr(self, mock_bbr):
    ...
    @patch(
        "cli.main.AdvancedOps.set_swappiness",
        return_value=OperationResult(success=True, message="Swappiness set"),
    )
    def test_advanced_swappiness(self, mock_swap):
