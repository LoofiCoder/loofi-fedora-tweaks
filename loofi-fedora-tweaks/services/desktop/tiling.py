...
Migrated from utils/tiling.py in v2.0.0 "Evolution".
"""
    ...
    def reload_config(cls) -> Result:
        ...
        try:
            if cls.is_hyprland():
                ...
                result = subprocess.run(["swaymsg", "reload"], capture_output=True, text=True, timeout=10)
            else:
        ...
        if cls.is_hyprland():
            for ws_num, ws_config in template["workspaces"].items():  # type: ignore[union-attr]
                ...
                for app in ws_config["apps"]:
                    config_lines.append(f"windowrulev2 = workspace {ws_num} silent, class:^({app})$")
            config_lines.append("")
        elif cls.is_sway():
            for ws_num, ws_config in template["workspaces"].items():  # type: ignore[union-attr]
                config_lines.append(f"# Workspace {ws_num}: {ws_config['name']}")
...
""")
            ...
            return Result(True, f"Dotfiles repo created at {repo_path}", {"path": str(repo_path)})
        except (subprocess.SubprocessError, OSError) as e:
            return Result(False, f"Error: {e}")
