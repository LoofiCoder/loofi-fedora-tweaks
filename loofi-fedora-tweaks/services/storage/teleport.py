...
Migrated from utils/state_teleport.py in v2.0.0 "Evolution".
"""
...
class StateTeleportManager:
        ...
        if shutil.which("code"):
            try:
                ...
                if result.returncode == 0:
                    state["extensions"] = [ext.strip() for ext in result.stdout.strip().split("\n") if ext.strip()]
    ...
    def capture_terminal_state(cls) -> dict:
        ...
        if os.path.isfile(history_file):
            try:
                ...
                history = [line.strip() for line in lines if line.strip()][-20:]
    ...
    def capture_full_state(cls, workspace_path: str = None) -> WorkspaceState:  # type: ignore[assignment]
        ...
        open_files = [os.path.join(ws_path, f) for f in vscode_state.get("open_editors", [])]
    ...
    def restore_git_state(cls, git_state: dict, repo_path: str) -> Result:
        ...
        try:
            ...
            if status_result.returncode == 0 and status_result.stdout.strip():
                return Result(
                    False,
                    f"Local changes detected in {repo_path}. Please commit or stash before restoring git state.",
    ...
    def apply_teleport(cls, package: TeleportPackage) -> Result:
        ...
        if ws_path and workspace.git_state.get("branch"):
            git_result = cls.restore_git_state(workspace.git_state, ws_path)
        ...
        if workspace.vscode_workspace.get("workspace_path"):
            vscode_result = cls.restore_vscode_state(workspace.vscode_workspace)
