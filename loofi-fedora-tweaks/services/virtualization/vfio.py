...
Builds on VirtualizationManager for low-level IOMMU/GPU queries.

Migrated from utils/vfio.py in v2.0.0.
"""

import os
import subprocess

from services.virtualization.virtualization import VirtualizationManager
...
class VFIOAssistant:
    ...
    def get_step_by_step_plan(cls, target_gpu: dict) -> list:
        ...
        steps = [
            {
                "step_number": 1,
                "description": (f"Add IOMMU and VFIO kernel parameters to GRUB. Append to GRUB_CMDLINE_LINUX: {kernel_args}"),
