...
Migrated from utils/vm_manager.py in v2.0.0.
"""
...
VM_FLAVORS: dict[str, dict[str, Any]] = {
    ...
    "fedora": {
        ...
        "vcpus": 2,
        "disk_gb": 30,
        "iso_url": ("https://download.fedoraproject.org/pub/fedora/linux/releases/41/Workstation/x86_64/iso/"),
        "needs_tpm": False,
...
class VMManager:
    ...
    def create_vm(cls, name: str, flavor_key: str, iso_path: str, **overrides) -> Result:
        ...
        cmd: list[str] = [
            ...
            str(vcpus),
            "--disk",
            f"path={disk_path},size={disk_gb},format=qcow2",
            "--cdrom",
            iso_path,
            "--os-variant",
            os_variant,
        ...
        if flavor.get("needs_tpm", False):
            if shutil.which("swtpm"):
                ...
                return Result(
                    False,
                    "Windows 11 requires swtpm for TPM 2.0 emulation. Install with: dnf install swtpm",
                )
